    ...
    if(tip*sdata+sdata<n&&tip<n)
    {
        ...
        //  __syncthreads();
           while (sdata<n) { 
                ...
                // visired[g_idata[indexes1[i+sdata]]]!=1&&
                  distance[e[n1[g_idata[indexes1[i]]]]]>distance[e[n1[g_idata[indexes1[i+sdata]]]]])
                     indexes1[i]=indexes1[i+sdata];
                // else if(i+sdata<n&&visired[e[n1[g_idata[indexes1[i+sdata]]]]]!=1&&visired[e[n1[g_idata[indexes1[i]]]]]==1)
                //         indexes1[i]=indexes1[i+sdata];
    ...
    if(tip*sdata+sdata<n&&tip<n)
    {
        ...
        //  __syncthreads();
           while (sdata<n) { 
                ...
                // visired[g_idata[indexes2[i+sdata]]]!=1&&
                  distance[e[n1[g_idata[indexes2[i]]]]]>distance[e[n1[g_idata[indexes2[i+sdata]]]]])
                     indexes2[i]=indexes2[i+sdata];
                // else if(i+sdata<n&&visired[e[n1[g_idata[indexes2[i+sdata]]]]]!=1&&visired[e[n1[g_idata[indexes2[i]]]]]==1)
                //         indexes2[i]=indexes2[i+sdata];
                if(threadIdx.x==0)
                 sdata*=2;
                __syncthreads();
        ...
        // indexes[threadIdx.x]= tip;
           while (sdata<TH_PER_BLOCK) {
              int i= 2*sdata*(threadIdx.x);
                    ...
                    if(tip*sdata+sdata<VERTICES&&tip<VERTICES-1)
    { 
                if(i+sdata<TH_PER_BLOCK) 
                {
                    ...
                    if(
                    visired[tis]!=1&&//tis umjesto i
                    g_idata[ti]>g_idata[tis])
                        indexes[i]=indexes[i+sdata];
                    else if(visired[tis]!=1&&visired[ti]==1)
                        indexes[i]=indexes[i+sdata];
